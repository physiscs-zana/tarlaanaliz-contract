...
## §0.A §13 — KR/SSOT belge denetimi + iki gün öz-denetimi (2026-09-07)

### Ne yapıldı

Altı uzman merceği (SWE · DL/ML · ürün-KR · veri mühendisliği · uzman portali ·
ops) bugünkü kod değişikliklerine karşı kanonik metinleri taradı; bir çürütücü
bulguları dosyaları açarak tek tek sınadı.

| | sayı |
|---|---|
| ham bulgu | 58 |
| elenen küme | 10 (`ZATEN_DOGRU` · `URUN_ETKISI_SIFIR` · `KAPSAM_DISI`) |
| ayakta kalan | 14 |
| merge edilen PR | 5 |

**PR'lar:** ct **#151** (v7.17.1, etiket `v7.17.1` basıldı) · work **#297** (rampa
+ metinler) · work **#298** (vendored re-pin) · plat **#538** (pano süzgeci +
çiftçi metni + rehber + platform re-pin). Üçü de **üretime dağıtıldı**.

### 🔴 Üç kanonik alan ölçümle çürütüldü

1. **`internal_use`** — `stress_ratio` DK-61 ile anomali kapısından emekli edildi;
   metin hâlâ *"eşiğin ALTINDAKİ karo derin incelemeye yollanır"* diyordu.
2. **`direction_and_range`** — *"tipik aralık 0–1"* yanlıştı **ve tüketicide
   ölçülebilir zarara dönüşmüştü**. `STRESS_RATIO_RAMP` aritmetiği:

   | değer | ne | renk |
   |---|---|---|
   | 0,26–0,31 | yayınlanmış **SAĞLIKLI** SCCCI aralığı | turuncu-kahve `#BB772E`–`#C78B34` |
   | 0,69 | PROSAIL fiziksel tavan | sarı-yeşil `#ADC44C` |
   | 1,00 | **taç dışı nötr klemp** = ölçüm YOKLUĞU | **en koyu yeşil** `#1A731A` |

   Sağlıklı taç "stresli", çıplak toprak "en sağlıklı" görünüyordu. Katman uzman
   inceleme yüzeyinde canlı → doğrudan yanlış-pozitif üretir.
   **Yerine yeni mutlak eşik YAZILMADI** (kanonik `interpretation_limits` ③
   kalibrasyonsuz mutlak eşiği geçersiz sayıyor): ölçek **sahne içi göreli**
   gerdirmeye çevrildi, geri-düşme rampasının durakları **eşit aralıklı** yapıldı,
   üst sınır fiziksel tavana (0,70) oturtuldu, taç dışı piksel rampaya **hiç**
   sokulmuyor (NaN → şeffaf).
3. **`measured_from` + `measured_producers`** — sekiz satır atfının **sekizi de**
   bayattı (sapma 500-1500 satır; ör. `:902` bugün `_resolve_pinned_ip`'e düşüyor).

### 🔴 Sağlıklı tarla panodan düşüyordu

| | davranış |
|---|---|
| yazıcı `_insert_timeseries` | yalnız `NO_RESULT`'ta erken dönüyor → satır **YAZILIYOR** |
| okuyucu `_VALID_RESULT_MODES` | `{FULL, PARTIAL}` — **altı yerde**, satırı **ELİYOR** |

Kusur **sessizdi**: KR-091 §6 boş panoyu hata saymıyor. Kol DK-61'den **önce
ulaşılamazdı** (550/550 karo anomali), yani kusur başka bir düzeltme onu
ulaşılabilir kılınca **doğdu**. Ölçüt artık `result_mode` değil **ölçümün
geçerliliği**: `INDICES_ONLY` + `anomaly == 0` dahil, `None` fail-closed dışarıda.

Çiftçi metni de ikiye ayrıldı: *"yalnız temel haritalar **üretilebildi**"* bir
**kısıt** anlatıyordu ve sağlıklı tarlalar artık o koldan geliyor.

### 🔴 Uzman rehberinde altı düzeltme

En ciddisi: *"tek bir karoda bile incelemenin sonucu red olur"* — **kod bunu
yapmıyor**. `expert_review_rules.aggregate_tile_verdict`: hepsi `not_a_finding`
ise `rejected`, **bir kısmı** ise `corrected`. O kod **2026-09-03'te tam bu kusur
için düzeltilmiş**, rehber metni geride kalmış. Yanlış metin uzmanı "yok" demekten
çekindirip **eğitim verisini bozuyordu**.

Ayrıca: `stresOraniAlti: 0.85` sabiti silindi · "sağlıklı karo sıfırdı" ölçümü
tazelendi (artık 25'in 4'ü · 80'in 22'si) · **"ölçülecek bitki yok"** kolu
anlatıldı (bitki hasta DEĞİL) · kapının sayısı ile kartın sayısının **aynı şey
olmadığı** yazıldı (tüm-piksel 0,317 · yalnız taç 0,656) · konum satırının aşırı
iddiası düzeltildi (piksel aralığı, coğrafi koordinat değil).

### 🔴 Bugün yazdığım üç yanlış (öz-denetim)

1. **"%16-21" aritmetik yanlış** — gerçek: 4/25=%16 · 22/80=%27,5 · **17/25=%68**.
   "~%70-80 tasarruf" hedefi Dicle'de **tutmuyor** (%32).
2. **"Telde ayırt edilebilir: (B) NO_RESULT + anomali > 0"** — `to_dict`
   `tile_counts`'ı yalnız `result_mode != NO_RESULT` iken serileştiriyor.
3. **"Bu dal BUGÜN ULAŞILAMAZ"** — DK-61 ile aynı gün ulaşılır oldu.

Ayrıca `_anomaly_filter` içindeki risk kaydı dosyayı **kendi kendisiyle
çelişkiye** düşürüyordu (*"kapı yeniden tasarlanmadan `apply_mask=True`
geçilemez"* derken 800 satır yukarıda geçiliyordu). Blok **silinmedi, öncülü
ölçüldü ve çürütüldü**: maske yerel kopyaya uygulanıyor, bant dizilerine
dokunulmuyor, Aşama-2 indeksleri **ham** bantlardan yeniden hesaplıyor.
Kilit: `TestMaskeAsama2yiKirletmez`.

### ⚠️ DK-61 kararını koruyan hiçbir kilit yoktu

`TestEsikIliskisi` imzadaki **varsayılanı** okuyup `>= 0.8` diyordu — ölü bir
ilişkiyi kilitliyordu. Eşiği 0,90 yapmak da yeşil kalırdı ama `stress_low`'un
kapıya geri sızması testi **kırmazdı**. `TestStresOlcutuKapidaDEGIL` ile
değiştirildi.

### ⚠️ İki ölçüm tuzağı (kendi hatalarım)

1. **CRLF, ikinci biçim.** Pin **HAM** blob hash'i ister. Normalize edince ALTI
   dosya değişti, oysa `git diff` **BEŞ** gösteriyor; fazladan gelen
   `drone_type.enum` iki sürümde de aynıydı — pini **doğruydu**, normalizasyonum
   onu bozuyordu (dosyanın kendi blob'unda **35 CRLF** var). `grep -c $'\r'`
   **0 dedi ve yanılttı**; CR'yi Python ile say.
2. **Yerel kapı CI'dan zayıf, yine.** Worker re-pin yerelde `4737 passed`, CI'da
   iki kırmızı: `metadata.lastUpdated` parite kapısı (kardeş depo ister) ve
   CHANGELOG zorunluluğu (workflow adımı, pytest kapsamında değil).

### Ölçüm

**16 mutasyon, 16 kırmızı, 0 kaçak** (rampa 7 · kapı kilidi 4 · pano/metin 5).
İlk turda **iki kaçak** vardı ve ikisi de gerçek test boşluğuydu: fikstürüm düz
diziydi (ikinci geri-düşme kolu testi ayırt edici olmaktan çıkarıyordu) ve PNG
imzasına bakmak **çağrı yerini** ölçmüyordu.

Dağıtım **dışarıdan** doğrulandı: pozitif kontrol (`Karo Yorumlama`) + yalnız
#538'le doğan `0,656`/`0,317` canlı pakette **var**, kaldırılan "stres oranı"
ibaresi **yok**. Worker `running healthy RestartCount=0`, RabbitMQ'ya bağlı.

### Açık kalanlar

- **Yer gerçeği yok** → hiçbir eşiğin duyarlılık/özgüllüğü ölçülemedi. `stress_ratio`
  emekliliği "bilgi eklemiyor" ölçümüne dayanıyor, "zararlı" ölçümüne değil.
- `retention_service.py:260` `result_mode="FULL_REPORT"` sabit kodlu (KR-090 md. 7-a
  yazıldı, **kod yazılmadı**); pencere ~709 gün sonra ulaşılabilir.
- `tile_counts` KR-093 kapalı listesine **eklenmedi** (bugün tüketicisi yok);
  geri açma tetikleyicisi kanonik metne yazıldı.
- Dedup: bağlanma ölçüldü (`pipeline.py:4379`), **canlıda grup oluşumu ölçülmedi**.
...
> **Bu turun tek cümlesi:** rehber sayfası uzmanın gördüğü ekranı anlatmıyordu;
> denetlendi, 12 bölüm olarak yeniden yazıldı ve rehberin anlattığı üç ekran
> öğesi (tarla referansı · tek güven bandı · ipucu kaynağı) arayüze taşındı.
> **plat #535 açık; merge ve deploy ürün sahibinde.**

### 1) ÖNCE GÜNCELLİK — bu not BEŞ GÜN bayattı

Ölçüldü (`git log --since=2026-08-31`): notun son güncellemesinden sonra
**ct #142 (7.16.0) · #143 (7.17.0)**, **work #283–#290 (9 PR)**, **plat #505–#533
(29 PR)** merge edilmişti; §0.A hâlâ "7.15.0, TUR 3-4" anlatıyordu. Edge pini
**7.15.0** (iki sürüm geride; sözleşme 7.16/7.17 edge baytını değiştirmiyor —
saf pin borcu). Canlı web: rehbere özgü Tailwind sınıfı (`list-decimal`, plat
#530) canlı CSS'te **var**, negatif kontrol 0 → rehber sayfası yayında.
Hafızadaki "uzman 5 karo görür" bayattı: `tile_selection.KANIT_KARO_SAYISI = 10`
(ürün kararı 2026-09-01).

### 2) "HOLİSTİK YAKLAŞIM" — belge YOK, yorum beyan edildi

Dört depoda ve hafızada bu adla belge yok (grep 0; worker CHANGELOG'daki
"holistik kapanış" ilgisiz bir denetim başlığı). **Bütüncül okuma** olarak
yorumlandı: karar tek sayıya değil, ekrandaki tüm sinyallerin birlikte
okunmasına dayanır. Ürün sahibi başka bir şey kastediyorsa düzeltilmeli.

### 3) DENETİM — dört mercek (57 bulgu) + bilim merceği (16 bulgu)

Çürütücü filosu oturum limitine takıldı (177 ajan → 173 hata); bulgular
günlükten kurtarıldı, sayısal iddialar **elle** (grep) doğrulandı, bilim
bulgularından ikisi iki bağımsız çürütücüden geçti. Ayakta kalanlar:

| sınıf | bulgu | kanıt |
|---|---|---|
| EKSİK | çok bantlı (NIR-K-Y) görüntü rehberde hiç yok — orada KIRMIZI = canlı, ısı haritasında KIRMIZI = düşük | `ReviewImagerySection.tsx` karoKatmanlari · `false_color_renderer.py` |
| EKSİK | karonun neden geldiği (kapı NDVI<0,55 ∨ NDRE<0,15 ∨ stres<0,85; çeşitlilik seçimi ≤10; en kararsız önce) | worker `pipeline._anomaly_filter`, `tile_selection.py`, plat `expert_portal.py` sıralama |
| TUTARSIZ | "komşu karolara göre" talimatı yürütülemez: karolar komşu değil, EN FARKLI olanlar; tarla haritası GÖRELİ (%2–%98) | `tile_selection.cesitlilik_secimi` · `IndexScaleLegend.tsx` |
| TUTARSIZ | NDVI merdiveni 0,15/0,30/0,50/0,75 üründeki HİÇBİR ölçekle örtüşmüyor (çiftçi 0,2/0,4/0,6/0,8 · kapı 0,55 · maske 0,40) — NDVI 0,52 rehberde "iyi", çiftçide "orta", worker'da "anomali" | `health_distribution.yaml`, `pipeline.py` |
| YANLIŞ | "HASTALIK DEĞİL kartları her tuzak için vardır ve her zaman gösterilir" — 6 bitkide 9-10 kart; kalibrasyon için hiç yok | `expert_labeling_cards/*.yaml` grep |
| TUTARSIZ | güven bandı: karo kartı 0,25/0,50/0,75, karar-sonrası panel backend 0,25/0,40/0,60; yorum "aynı" diyordu — güven 0,45 karar öncesi "düşük", sonrası "orta" | `ReviewImagerySection.guvenBandi` · `expert_review_rules._BANT_SINIRLARI` |
| EKSİK | backend tarla referansı (`field_mean_ndvi/basis/coverage_ratio`, #499) web'de **hiç okunmuyordu** | `git log -S field_mean_ndvi -- web/src` boş |
| EKSİK | `hint_source` (7.17.0) backend'de var, panelde yok; `std` worker'da var, tabloda yok | `expert_portal.py` ModelOpinionTile · `tile_crop_renderer.py` |
| BİLİM | NIR↓+R↑ imzası çıplak toprakta da aynı (toprak NDVI 0,19); antosiyanin yeşil yansımayı TERSİNE düşürür; gölge/bulut MASKELENMEZ, gri = yalnız ölçüm yok; NDRE'nin "önce"si NDVI'ya göredir; çeltikte su parlaması | çalışma alanı kökündeki spektral bant raporu (git dışı) §1-2, `index_heatmap_renderer.py`, `health_distribution.yaml` |
| KURAL | eski test 13 mutasyondan 11'ini kaçırıyordu; KR-025 kapısı üç sözcüktü ("fungisit önerilir" CI'dan geçti) | mutasyon ölçümü (mercek D) |

Çürütülen/kabul edilen sadeleştirmeler: "oran olduğu için kayabilir" (gölge)
ve "tek ölçek" ifadesi — metin yine de düzeltildi.

### 4) NE YAPILDI (plat #535, 9 dosya, +1124/−238)

* Rehber 12 bölüm; **sayılar tek sabit dosyasından** (`rehberSabitleri.ts` —
  her sabit worker/platform kaynağıyla yorumlanmış; test aynı sabitleri sayfada arar).
* `guvenBandi.ts`: backend aynası + ASCII→Türkçe etiket (panel uzmana "dusuk" basıyordu).
* `ReviewImagerySection`: tarla referansı satırı (null ise yok), sapma sütunu,
  tek güven bandı, GNDVI "neye bakın" metni rehberle hizalı, gri/gölge metni,
  Rehber bağlantısı, "sarararma" düzeltildi. `ModelOpinionPanel`: ipucu kaynağı.
* Test: drift kilidi formül + "ne ölçer"; NDVI satırlarının tamamı; KR-025 kök
  tabanlı regex + POZİTİF KONTROL; bölüm sayacı alt VE üst sınır; karar
  etiketleri `DECISION_LABELS`'tan.
* Ölçüm: tsc 0 · eslint 0 · jest **57/579** (temel 559) · `next build` 69 sayfa ·
  **mutasyon 16/16** + M0 yeşil. ⚠️ İlk turda `cmd /c pnpm test -- "a|b"` deseni
  `cmd`'de BORU oldu (rc=255) — iki ölçüm geçersizdi, PowerShell'de tek tırnakla
  yeniden koşuldu.

### 5) WORKER KARŞILAŞTIRMASI — rehber ↔ worker (yalnız karşılaştırma, kod değişmedi)

| rehberin bilgi sınıfı | worker | kanıt |
|---|---|---|
| NDVI · NDRE · stres oranı | **kullanır** (Aşama-1 kapısı 0,55/0,15/0,85) | `pipeline._anomaly_filter` |
| NDRE NDVI'dan önce | **kullanır** (kanıt sınıflandırıcı ağırlık 0,55 / 0,35) | `evidence_sub_domain_classifier.py` |
| ham 4 bant + karo İÇİ uzamsal yapı | **kullanır** (tensör G,R,RE,NIR,NDVI,NDRE,NDWI; 512×512) | `channel_spec.py`, `model_registry.yaml` |
| bant istatistiği (ort/std/p25/p75) | **yalnız üzüm LR taban modelinde** | `feature_extraction.py`, `model_registry` grape_disease_baseline_v1 |
| fenoloji / takvim / BBCH | **kullanır** (sert kapı + ağırlık) | sınıflandırıcı `calendar_months`, `phenology_bbch` |
| kart alanları | **kısmen**: detectability/reliability/ndvi_infected/ndre_infected; `pattern`, `typical_location`, `distinguishing` OKUNMUYOR | sınıflandırıcı `card.get(...)` sayımı |
| kalibrasyon bağlamı | **kullanır** (`absolute_scale_valid`; imza atlası PANEL/DLS2 ister) | `reporting_agent.py`, `spectral_signatures.yaml` |
| **karolar arası desen / komşuluk / tarla referansı** | **KULLANMAZ** — karo `mean()`, kapı mutlak eşik; tarla ortalaması yalnız raporlanır | `pipeline.py` grep neighbor/komsu 0 |
| **zaman ekseni (önceki uçuş)** | **KULLANMAZ** (`temporal` yalnız docstring); 16 kart çok-zamanlı ister | `pipeline.py:1306`, kartlar `multi_temporal_required` |
| **gölge / bulut** | **KULLANMAZ** — piksel maskesi yok; `saturation_cloud_checker` çağıransız | grep shadow_mask |
| doyum | yalnız ≥0,99 klip uyarısı (radyometrik), NDVI doyumu ele alınmaz | `pipeline.py` ~3290 |
| GNDVI | hesaplanır, tensöre GİRMEZ; NDWI (= −GNDVI) girer | `channel_spec.py` |
| toprak maskesi | çiftçi dağılımında 7 ağaçlı mahsulde NDVI≥0,40; **uzman karolarında YOK** | `crops/*.yaml`, `health_distribution.py` |

Özet: worker rehberin **sayısal** sınıflarını (indeks, eşik, fenoloji, kalibrasyon)
kullanır; rehberin uzmana öğrettiği **bütüncül** sınıfları (desen, komşuluk, tarla
referansı, zaman, gölge ayrımı) **hiç** kullanmaz. İkisi tasarım gereği ayrıdır
(worker "bakılmalı" der, uzman "ne" der); ama worker tarafında kapatılabilir en
ucuz boşluk **tarla referansına göre normalize edilmiş kapı** ve **kart `pattern`
alanının hiç okunmaması**dır — ürün kararı bekler.

### 6) İKİNCİ TUR — rehberin verdiği sözler ürüne bağlandı (plat #536, edge #86)

Rehber uzmana iki söz veriyordu, ekran ikisini de tutmuyordu; ikisi de aynı
sınıf (**üretici var, tüketici yok**) ve ikisi de kapandı:

| söz | ölçüm | çözüm |
|---|---|---|
| "kartların görülme aylarını eşle" | ekran yalnız **atanma** tarihini basıyordu; `datasets.capture_date` VARDI (ingest `manifest.captured_at`ten türetir), uzman ucuna hiç taşınmıyordu | uçuş tarihi karo listesinin üstünde; kayıtlı değilse satır çizilmez |
| "kart çok-zamanlı gözlem istiyorsa tek uçuşla onaylamayın" | `multi_temporal_required` **16 kartta true**, arayüz tipinde tanımlı, kart gövdesi hiç basmıyordu | kartta "⏱ Çok-zamanlı gözlem gerekir" uyarısı; metin TEK sabitte, rehber testi kaynağından okur |

Yeni yardımcı `dataset_values_for_result` AYNI satırdan N kolon okur (tek sorgu):
iki ayrı çağrı aynı bağı iki kez sorgular ve iki alanın **aynı uçuştan** geldiği
garantisini kaybederdik; sorgu sayısı değişmediği için çağıranların fikstür
sırası korunur.

🔴 **Üç ölçüm tuzağı bu turda ayrı ayrı yakalandı:**
1. **Fikstür sorgu biçimiyle hizasızdı** — eski fikstürler `_scalar_one`
   kuruyordu, kod `.scalars().first()` çağırıyordu: manifest değeri koda **hiç
   ulaşmıyordu**, testler yeşildi ama kalibrasyon bağlamını ölçmüyordu.
2. **Mutasyonun ikisi kaçtı** — uç testleri `session.execute`i mock'ladığı için
   SORGUYU göremiyor; "ikinci kolonu düşür" oradan geçiyordu. Yardımcıya kendi
   testi yazıldı (sorgu metni + çağrı sayısı + açılabilirlik) → 15/15 yakalandı.
3. **Tam paketteki tek kırmızı ORTAMDI** — izole çalışma ağacında `contracts/`
   submodule checkout edilmemişti ve KR-093 kapısı fail-closed kapanıyordu.
   Submodule init edilince yeşil; kodla ilgisi yoktu.

**edge #86:** pin 7.15.0 → 7.17.0. Edge'in vendor'ladığı 10 dosyanın 10'u iki
etikette bayt-özdeş (kendimiz ölçtük) + pozitif kontrol (`analysis_result.v1`
değişmiş görünüyor, yani ölçüm aracı çalışıyor). SemVer 1.7.0 ve hash bloğu
korunur; dört kapı rc=0.

### 7) AÇIK KALEMLER (hâlâ YAPILMADI, beyan)

* Bitkiden bağımsız gölge/doyum/kalibrasyon uyarı kartları (`general_abiotic`).
* Önceki uçuşla karşılaştırma (backend bilerek yok: kapsam normalizasyonu şart).
* **BBCH/fenoloji evresi** uzman ekranında hâlâ yok (uçuş tarihi geldi, evre gelmedi).
* Worker tarafı: kart `pattern` alanı hiç okunmuyor; kapı tarla referansına göre
  normalize edilmiyor — ikisi de ürün kararı bekler.
### 8) DAĞITIM — YAPILDI ve ÜÇ KATMANDA ÖLÇÜLDÜ (2026-09-06, ürün sahibi devretti)

Ürün sahibi *"sen merge yap ve dağıtım yap"* dedi; döngü baştan sona koşuldu.

| katman | doğrulama |
|---|---|
| main | plat `d4de5fd7` (#536) · ct `f45ccd3` (#144) · edge `83fa159` (#86); dördü de squash-merge |
| üretim deposu | `git pull --ff-only` + `submodule update --init --recursive` → `d4de5fd7`, submodule `v7.17.0` temiz |
| dağıtım | `deploy_prod.sh --check` GEÇTİ (`CONFIG_OK`, `RASTER_OK 1.26.4 1.4.4`), sonra tam koşum: 7/7 adım, backend `/health` **200**, web **200**, `DAGITIM TAMAM` |
| canlı web | CSS özeti `982c02e0…` → **`ac154131…`** (yerel derlemeyle AYNI); yeni rehber imzası `list-style-type:circle` **VAR**; pozitif kontrol `decimal` var, negatif kontrol yok |
| backend konteyneri | `dataset_values_for_result` **2**, `capture_date` **1** (yeniden oluşturulmuş konteynerde ölçüldü) |
| hata | backend 15 dakikada **178 satır akış** (kanal ölü değil) · `ERROR|CRITICAL` **0** · pozitif kontrol `INFO|WARNING` **179** |

🔴 **ÜÇ MEKANİK DERS** (ayrıntı: hafıza `platform-merge-deploy-etmez`):
1. **Uzun uzak derleme SSH oturumuna bağlanmamalı** — ilk `--check` koşumu web
   derlemesinin ortasında `Connection reset by peer` ile düştü (rc=255). Sunucu
   suçsuzdu: yük 0.27, 10 GB boş bellek, **OOM 0**, site 200 dönmeye devam etti
   (`--check` çalışan yığına dokunmuyor — belgelenen davranış doğrulandı).
   Doğrusu `setsid nohup … > /tmp/x.log` + terminal imzayla yoklama.
2. **Squash-merge yığılı dalı çakıştırır** — #535 squash'lanınca yeni SHA doğdu,
   aynı içeriği taşıyan #536 `CONFLICTING` oldu. Çözüm: `git reset --soft
   origin/main` + tek commit + `--force-with-lease` (ağaç değişmedi, taban ilerledi).
3. **Yığılı PR'da CI HİÇ koşmaz** — platform iş akışları yalnız tabanı `main`
   olan PR'ı tetikler; tabanı sonradan değiştirmek de yetmez (`edited` olayı).
   `gh pr close` + `gh pr reopen` ile tetiklendi.

⚠️ **Worker DAĞITILMADI ve gerekmedi:** bu turda worker deposunda değişiklik yok
(`ebeb11c`, #290'da duruyor). Edge de yalnız pin dosyasını değiştirdi; dağıtım gerektirmez.

### 9) AKADEMİK DOĞRULAMA — rehberin 67 iddiası hakemli literatüre karşı sınandı (plat #537)

Ürün sahibi *"yaptıklarının doğruluğunu akademik kaynaklardan denetle"* dedi.
Rehber sayfası on kümeye bölündü; her küme bağımsız bir ajanla, **DOI'li hakemli
kaynağa** karşı sınandı (workflow `wf_c5d651c7-d12`, 10 ajan, 10/10 sonuç döndü).
...
🔴 **İKİ YANLIŞIN İKİSİ DE BU OTURUMDA SAYFAYA BENİM EKLEDİĞİM CÜMLELERDİ.**
Yani *"denetleyip yeniden yazdım"* demek *"doğru yazdım"* demek değildi — kendi
yazdığımı kaynağa karşı sınamadan üretime gönderdim ve **ikisi üretimde bir gün
canlı kaldı**.

| yanlış | gerçek | kaynak |
|---|---|---|
| Mavi bant "pigment bozulmasını **en erken** gösteren bölge" | Mavi, klorofil+karotenoidin **en güçlü yuttuğu** bölge; yansıma erken doyar, duyarlılık **düşüktür**. Karotenoid duyarlılığı ~**510 nm**'de tepe yapar — mavi pencerenin (450–475 nm) **DIŞINDA**. Erken haberci **kırmızı kenar**. | Gitelson, Zur, Chivkunova & Merzlyak 2002, *Photochem. Photobiol.* 75(3):272–281 |
| Aykırı değerde "**sapma şişmez**" | Standart sapmanın **kırılma noktası %0**: tek bir aykırı değer onu istenildiği kadar büyütebilir. | Rousseeuw & Hubert 2018, WIREs DMKD 8(2):e1236 |

Her iki kaynağı da ajandan **bağımsız olarak kendim** doğruladım (WebSearch +
ar5iv tam metin); ajanın atfı uydurmadığı ölçüldü.

**#537'de düzeltilen sekiz KISMEN:** antosiyaninde GNDVI *düşmez değil, TERSİNE
ARTAR* (karo sağlıklı görünür) · kırmızı kenar "doymaz" mutlak ifadesi · kırmızı
bant düşük klorofilde bile doyar · NIR kurumada **ARTABİLİR** · NDVI merdiveni
ürünün kendi sınıflandırması (doyum **yaprak alanı indeksi ikiyi aştığında**
başlar, "0,75–0,85"te değil) · örtü sıklaştıkça kırmızı **KOYULAŞIR** ·
kombinasyon imzaları tek nedene **ÖZGÜ DEĞİLDİR** · desen olası nedeni
**DARALTIR, kesinleştirmez**.

Ölçüm: 10 yeni kilit testi, her biri eski yanlış ifadeyi **negatif kontrolle**
kapatır. **Mutasyon 10/10 KIRMIZI**; mutasyonun *uygulandığı* ayrıca ölçüldü;
kaynak **bayt-özdeş** geri alındı (`sha256` eşit, `55864aff716a80b6`); pozitif
kontrol mutasyonsuz ağaçta `rc=0`. type-check 0 · lint 0 · jest **596/596** ·
next build 0.

⚠️ **KAPANMAYAN:** kalan **26 KISMEN** iddia bu turda düzeltilmedi (yanlış
değil, eksik nüanslı). Bir iddia için hakemli sayı **bulunamadı**: *"sağlıklı
dokuda GNDVI sarı (0) – açık yeşil (+0,3) bandında kalır"* — NDRE yarısı için
kaynak var (Yang ve ark. 2020, *Front. Plant Sci.* 11:927), GNDVI yarısı için
yok. Sayfa bu cümleyi hâlâ söylüyor.

🔴 **KALICI DERS:** *"literatüre dayanıyor"* diye yazılan her cümle, **yazıldığı
...
* ~~#5 `saturation_cloud_checker` onarımı çürütülmedi~~ → **§11'de çürütüldü ve ÇÖKTÜ.**

> ⬇️ **Bu listenin TAMAMI §11'de kapatıldı** (biri uygulandı, dördü ölçümle düştü).

### 11) KALAN BEŞ KALEM KAPATILDI — biri UYGULANDI, dördü ÖLÇÜMLE DÜŞTÜ

Ürün sahibi *"kapanmayanları yap"* dedi. Sonuç: beş kalemden **biri** indi,
**dördü** ölçümle düştü. Düşenler tembellikten değil, **kanıttan** düştü.

| kalem | akıbet | gerekçe (hepsi ölçülmüş) |
|---|---|---|
| iki tam koşum | ✅ **YAPILDI** | §10'daki düzeltmeyi doğurdu: maske uçtan uca **hiçbir şeyi** değiştirmiyor |
| **#1** kapı-kipi damgası | ⛔ DÜŞTÜ | çürütücü: tüketici yok · `_empty_response` `NO_RESULT`'ı sabitliyor · mandal ilk gün kırmızı |
| **#4** uzamsal seçim | ⛔ DÜŞTÜ, **yerine W-3 indi** | öncülü ölçümle doğrulanmadı: seçilen 10 karo tarlanın **tamamına yayılmış** (5/5 satır, 5/5 sütun). Gerçek eksik seçim algoritması değil, **konumun uzmana hiç gitmemesiydi** |
| **#5** QC onarımı | ⛔ ÇÖKTÜ | iki çürütücü **beş ÖLÜMCÜL** kusur ölçtü (aşağıda) |
| **#6** QC bağlama | ⛔ ERTELENDİ | #5 çökünce ön koşulu yok |
| **#7** göreli eksen | ⛔ DÜŞTÜ | §10'daki `stress_ratio` bulgusu: stres %100 bağlarken göreli NDVI ekseni **hiçbir şey** değiştirmez |

#### ✅ W-3 İNDİ ve DAĞITILDI — kanıt karosunun KONUMU (work #292, `6c0e8fe1`)

Aradığım kusur sınıfı hep *"üretici var, tüketici yok"*tu. W-3 bunun **tersi**:

| parça | durum |
|---|---|
| sözleşme | `ExpertEvidenceTile.bbox` **tanımlı**, açıklaması aynen *"Null until the producer fills it (W-3)"* |
| tüketici | uzman portali "Konum (piksel)" satırını **basıyor**; dosyanın kendi notu: **11 sonucun 11'inde de boştu** |
| veri | karo sözlüğünde `row/col/height/width`, yorumu *"W-3 için, DOLGU ÖNCESİNDE"* |
| **üretici** | **yoktu** |

Uzmandan *"tarladaki deseni"* okuması isteniyordu ama karonun tarlanın
**neresinden** geldiği söylenmiyordu. Üretici bağlandı; **sözleşme değişikliği
gerekmedi**. Ölçü **dolgu öncesidir** (kenar karosu görüntünün dışına taşmasın).

🔴 **Mutasyon İKİ KEZ testimi çürüttü:**
1. Bileşim testim `tr.bbox`'ı **elle** atıyordu → `tile_data → TileResult`
   halkasını hiç geçmiyordu; o satırları iki yerden de silince **tüm testler
   yeşil kaldı**.
2. Mutasyonu **tek tek** koşturunca LR yolu kırmızı, **MC-dropout yolu yeşil**
   döndü — yani fıstık gibi **eğitimsiz** ürünlerin geçtiği **asıl üretim yolu**
   korumasızdı.

Ölçüm: 7 test · mutasyon **7/7 KIRMIZI** (iki kurulum yeri **ayrı ayrı** da
sınandı) · kaynak **bayt-özdeş** · pytest **4832** · ruff 0 · mypy sıfır ekleme ·
CI 5/5 · dağıtıldı ve çalışan süreçte doğrulandı (`RestartCount=0`, üç üretim
kuyruğu 1 tüketici).

#### ⛔ #5 QC ONARIMI ÇÖKTÜ — beş ölümcül kusur (hepsi gerçek veriyle ölçüldü)

1. **Ölçek vetosu işe yaramaz.** `[0,005 , 1,5]` aralığı 300 kat geniş, ölçülen
   ölçek hatası ~6 kat → yanlış ölçeği **onaylar**. Alt sınır zaten bu veriyi
   *geçirmek* için seçilmiş (döngüsel gerekçe).
2. **Bulut kolu ateşlenemez.** `nir > 2,5·p95` eşiği 0,2023; **sahne maksimumu
   0,1576**. Dokuz fizikî bulut senaryosunun sekizi eşiğin altında.
3. **Doyum kolu yapısal olarak kör.** Aynı uçuşun ham DN sürümünde
   `DN ≥ %98` olan **194 piksel var**; kalibre kardeşinde **0,000000**
   raporlanıyor. Kalibrasyon 65535'i 0,20'ye eziyor.
4. **"Ölçek-değişmez" iddiası aynı uçuşta çürüdü:** `k=0,35` kolu kalibre
   sürümde %6,76, ham sürümde %0,75 → **9 kat** fark.
5. **Gölge kolu gerçek gölgeyi kaçırır.** Enjeksiyon deneyi: NIR×0,20 → %99
   yakalanıyor, ×0,40 → **%0**. Tasarımın kendi gölge tanımının (güneşlinin
   %20-40'ı) **üst yarısı görünmez**.

⚠️ Kalıcı sonuç: bu üründe **mutlak yansıma yoktur, yalnız oranlar geçerlidir**.
QC modülü mutlak eşikle onarılamaz; ya fiziksel çapa (panel/DLS2) ya da
doyumun **sensör/DN uzayında** ölçülüp taşınması gerekir. Ürün kararı.


### 12) EŞİK RASTGELEYDİ — maske AÇILDI, stres ölçütü EMEKLİ (work #294, #295)

🔴 **Ürün sahibi açıkladı: `stress_ratio < 0,85` eşiğini RASTGELE yazmış.** Bu,
§10-§11'deki bütün bulmacayı kapattı ve eşiği değiştirmeyi meşru kıldı.

#### Bilimsel karşılık ARANDI, BULUNAMADI

Dört bağımsız türetme yolu (literatür · fizik/PROSAIL · dayanıklı istatistik ·
ürün bütçesi) + iki adversaryal denetim (34 DOI Crossref'te tek tek doğrulandı,
**uydurma atıf yok**):

1. **Yapısal tavan.** Vejetasyonda `RE > R` daima (**%99,99–100**) → oran daima
   1'in altında. PROSAIL + M3M (kırmızı-kenar **730 nm**): fiziksel tavan
   **0,66–0,69**; 3445 koşumun **sıfırı** 0,85'e ulaşıyor.
2. **İndeks meşru, eşik değil.** `NDRE/NDVI` = **SCCCI**; yayınlanmış sağlıklı
   aralık **0,26–0,31**, bizim taç ortancamız **0,275–0,319** — tam içinde.
   **Yayınlanmış mutlak eşik YOK**; literatür oranı hep GÖRELİ kullanıyor.
3. **Hiçbir eşik çalışmıyor.** Taç-içi stresle maskeli tabana eklenen karo:
   0,24 → **+0** · 0,26 → +2 · 0,27 → +9 · 0,30 → +40. Savunulabilir her eşik
   **kaldırmayla özdeş**; canlı tutan tek değer 0,30 = dağılımın **medyanı**.
4. **Bağımsız bilgi taşımıyor.** Taç-içi stres, kapının zaten okuduğu
   (taç NDVI, taç NDRE) ikilisiyle **R² = 0,963–0,968** açıklanıyor.

#### Sırayı ÖLÇÜM belirledi

| yol | Aşama-2 tasarrufu | maliyet |
|---|---|---|
| yalnız eşik (maske kapalı) | **%0** | 4 depoluk re-pin töreni |
| maske + eşik | %100 → **%16–27** | sağlıklı-tarla yolu ön koşul |

Üretimde iş başına **32,4 karo** MC-Dropout koşuyordu; tasarım hedefi 6,5–9,7.
Ödül tamamen maskedeydi → önce sağlıklı-tarla yolu (#294), sonra maske (#295).

#### ✅ ADIM 1 — sağlıklı-tarla yolu onarıldı (work #294, `b726bdd3`)

`if not tile_results` dalı iki farklı durumu tek cevaba indiriyordu:
* **veri yalanı**: `tile_count_anomaly = 0` sabitiydi; anomali karoları düşmüşse
  bu yalandı ve ML-03 yalnız `anomaly_count > 0` iken cezalandırdığı için
  **kısmi düşme `PARTIAL_REPORT`'a inerken TAM düşme temiz görünüyordu**.
* **kip çelişkisi**: dalın kendi yorumu *"INDICES_ONLY … honours them"* diyor ve
  `index_maps_raw` dolduruyordu ama kip `NO_RESULT` kalıyordu. Kanonik sözleşme
  çözdü: index_maps *"always shared in PARTIAL and INDICES_ONLY modes"*.

Artık telde ayırt edilebilir: sağlıklı → `INDICES_ONLY` + anomali 0 ·
tam düşme → `NO_RESULT` + anomali > 0 (fail-closed korundu).

#### ✅ ADIM 2 — maske açıldı, stres emekli (work #295, `103cf33d`, DAĞITILDI)

**Kararı veren ölçüm** — bugün uzmana giden 10 kanıt karosunun kaçı gerçekten
anomali: karaburun **0/10** · karaburun2 **1/10** · dicle 6/10. Yani maskesiz
kapı uzmanı neredeyse **saf gürültüyle** besliyordu.

| veri | karo | anomali | sağlıklı | kanıt | Aşama-2 |
|---|---|---|---|---|---|
| karaburun | 25 | 4 | **21** | 2 | **%16** |
| karaburun2 | 80 | 22 | **58** | 10 | **%27,5** |

Kodun kendi *"~%70–80 hesap tasarrufu"* hedefi **ilk kez** tutuyor.

Stres artık **taç pikselleri üzerinden** ortalanıyor (ham hesap + **çıktı**
süzgeci); `stress_ratio` karo sözlüğüne kaydediliyor. İndeks, `index_maps` ve
`WATER_STRESS` vekil katmanı **dokunulmadı** — emekli olan yalnız kapıdaki rolü.

#### ⚠️ İKİ UYARI (ikisi de ölçülmüş)

* **Zaman serisi kırıldı.** `tile_count_healthy` bugüne kadar **hep 0**'dı
  (üretimde ölçüldü). Kırılma **düzelmedir**; log kipi damgalıyor
  (`anomaly_gate_vegetation_masked=true`, `…stress_criterion=RETIRED`).
* **Kanıt karosu 10 → 2 (küçük tarla) gerileme DEĞİL** — bugünkü 10'un **0**'ı
  gerçekten anomaliydi. Platformda kanıt sayısını doğrulayan kod **yok**
  (`MIN_PATCHES_PER_REVIEW` yalnız uzman kotasından sayfa boyutu türetiyor).

#### ⛔ ÜÇ HİPOTEZ ÖLÇÜMLE ÇÜRÜDÜ

*"Zaman serisinde %89 boşluk var"* (**yanlış** — benzersiz kısıt
`uq_ts_field_mission`; 19 sonuç 2 tarla-görev çiftine ait, eksik satır **0**) ·
*"boşluk özellik yaşından"* (**yanlış** — seri ilk sonuçtan beri canlı) ·
*"`NO_RESULT` atlamasından"* (**yanlış** — sonuçlar `PARTIAL_REPORT`/
`INDICES_ONLY`). **Ders:** "N sonuç ama M satır" boşluk kanıtı değildir; önce
benzersiz kısıtı oku.

#### KAPANMAYAN

* **Sözleşme düzyazısı bayat**: `analysis_type.enum.v1.json` →
  `metadata.indexDefinitions.stress_ratio.internal_use` hâlâ
  *"`_anomaly_filter(stress_ratio_threshold=0.85)`"* diyor. Kanonik + worker +
  platform üç kopya; **re-pin töreni gerekiyor**.
* **Saklama geri-doldurması** `result_mode="FULL_REPORT"` sabit kodlu
  (`retention_service.py:260`). Gerçek kusur ama pencere **730 gün**, en eski
  sonuç 3 haftalık → **~709 gün ulaşılamaz** ve maskeyle ilişkisi yok.
* Yer gerçeği hâlâ yok: hiçbir eşiğin **duyarlılık/özgüllüğü** ölçülemedi.

---

## 0.B — (2026-08-31, **yirmi besinci oturum: EYLEM PLANININ TAMAMI (TUR 1 · 1.5 · 2 · 3 · 4) UYGULANDI · AYRI EKSEN · 7.15.0**)
...
Olculebilir iddialar tek tek sinandi; **ucu kusurluydu**, hicbiri **sonucu**
curutmedi. Ayrinti **§0.B (14)**. Kisaca:
(a) iki platform dosyasi **bayat checkout'tan** okunmustu (3075/2310 vs 3214/2361)
(b) FAISS "100 vektor" bugun **yeniden uretilemiyor** (157; kayitlar nottan once)
(c) "PR ACILMADI" bayat. **Oz-denetim dorduncusunu buldu:** dosya basligi bir
oturum geride (#134).

### 2) TUR 1 — W-4 · W-8 · P-3 (work #277, plat #501)

| kalem | sonuc |
|---|---|
| **W-4** | `unknown_anomaly` "ilkini al" -> **<=5 cesitlilik secimi** (L-infinity, `[ndvi,ndre]`) |
| **W-8** | eskalasyon `tile_id` tasir (alan **zaten vardi**, uretici yoktu) |
| **P-3** | kota defteri gosterilen karoyu sayar |

**Esik uydurulmadi:** `_ASGARI_IMZA_FARKI = 0.01` uretim verisinden secildi
(`inference.db`, **15 is / 493 karo**): 0.0-0.05 arasi her esik 15/15 iste **5
karo** verdi, F1(c)'nin 0.15'i 2-3'e dusurdu. **Tam ozdes cift uretimde VAR.**

### 3) W-4'un BEYAN EDILMEMIS SONUCU -> AYRI EKSEN (7.14.0)

Oz-denetim yakaladi: `detections` **ayni zamanda ciftci yuzeyinin kaynagi**
(`_normalize_detections` — tekillestirme YOK) ve ciftciye `tile_id`/`bbox`
**verilmez** (KR-071). Bes kanit karosu oraya yazilinca ciftci **bes ayirt
edilemez satir** gorecekti. Bugun tetiklenmiyordu (uretimde hicbir gorev `DONE`
degil) ama ilk uzman onayinda canlanirdi.

> **KURAL (7.14.0): UZMAN EKSENI BUYUYUNCE CIFTCI EKSENI BUYUMEZ.**
> `detections` = CIFTCI · `expert_evidence` = UZMAN

Urun sahibi karari: **ayri eksene ayir** (kalici cozum). Uygulandi:
ct **#135** + **#136** (`v7.14.0`), work **#278**, plat **#502**, edge **#83**.

### 4) TUR 1 devami — W-7 dort bant (work #278)

Karo basina **NDVI · NDRE · NDWI · GNDVI** isi haritasi + bant istatistigi.
**Olcek SABIT [-1,+1]** — platformun tarla dosemeleri **goreli** (%2-%98)
renklendirir; ikisi ayni renkle ayni seyi **anlatmaz**.
RE bandi yoksa NDRE **uretilmez** (uydurulmaz).

### 5) TUR 1.5 — W-9 (work #279): IKI mekanizmayi birden acti

`disease_class` alani vardi, **ureticisi yoktu** (uretimde **157/157 None**).
Bu tek eksik ikisini birden olu birakiyordu:
(a) `disease_hint` -> her karo `unknown_anomaly`
(b) **`atlas.invalidate`** -> `if not meta.disease_class: return 0`, REJECT
gelse bile L1 kaydi **hic silinmiyordu**.

**§0.B (11)'in "REJECT -> atlas zinciri calisiyor" olcumu IZOLE nesnelerleydi**
(notun kendisi bunu beyan ediyor); **uretimde kapi hep atliyordu**.

O-4 gevsetilmedi: kaynak yalniz `verdict == CORRECTED`. Kanonik kural da
`corrected_class`'i **yalniz o verdiktte zorunlu** kiliyor (KR-029) — olculdu.
Sira: **Adim 5d'den SONRA** (once kossa, duzeltmeli bir REJECT'te atlas
**yeni** sinifi gecersiz kilardi). Bir test bunu cagri sirasiyla kilitler.

### 6) TUR 2 — W-3 konum (work #280) + P-1 yuzey (plat #503)

`bbox` uretimde **11/11 bostu**. Artik **piksel** bbox tasiniyor ve uzman
ekrani `bbox` · `index_map_urls` · `band_stats` sunuyor.
Kenar karolari `tile_size`'a **doldurulur**; bbox **padding oncesi** olcuden
turer — yoksa karo goruntunun **disina** tasardi (test literal degerle kilitler).

### 7) URETIME DAGITILDI — ve DOGRULANDI

Sira kritikti (olculdu: uretim imaji 2026-08-30 tarihliydi): **once platform,
sonra worker**. Tersi olsaydi worker kaniti yeni eksene yazar, eski platform
`findings`'ten okur ve uzman **5'ten 1'e** duserdi.

| katman | dogrulama |
|---|---|
| platform | `deploy_prod.sh --check` gecti -> tam dagitim rc 0, backend+web healthy, uclar 200 |
| goc | kolon `expert_evidence` (jsonb) **var**, alembic head `analysis_results_expert_evidence` |
| backend | kosan konteynerde `_kanit_ekseni` x2, pin **7.14.0** |
| worker | surec **19:11:13** / dosya **19:10:55** (sonra), pin **v7.14.0**, yeniden baslatma sonrasi **0 ERROR** |
| geri-dusus | 16 sonucun **0'inda** kanit ekseni dolu, **11'inde** `findings` -> eski incelemeler **goruntusuz kalmadi** |

`deploy-staging.yml` bir **ISKELET** — her adimi yorum satiri, hicbir sey
dagitmiyor. Gercek yol: sunucuda `scripts/deploy_prod.sh`.

### 8) HAFIZADAKI BIR KURAL OLCUMLE CURUDU — surum toreni SIRASI

*"Etiket EN SONA"* bu duruma **uymuyor**: worker CI sozlesmeyi **pinli
etikette** checkout ediyor, etiket yoksa tuketici PR'i **gecemiyor**; kanonik
kapinin kendi mesaji da *"merge sonrasi `v7.14.0` basilmali"* diyor.
**Dogru sira: bump merge -> ETIKET -> tuketici re-pin'leri.**

### 9) KENDI URETTIGIM KUSURLAR (hepsi kapandi, hicbiri gizlenmedi)

1. **2 kirmizi kontrolle merge ettim** (#135) — arka plan `gh pr checks --watch`
   komutunun **cikis kodunu** "kontroller yesil" sandim; `master` kirildi,
   #136 ile kapandi. Artik **kontrol DURUMLARI** okunuyor.
2. **`tools/*.py` kosturdum, `pytest` kosturmadim** — `validate.py`
   `additionalProperties`'i kabul ediyor ama test `$defs` icin
   `unevaluatedProperties` istiyor. Kusur **kodda degil surecte**.
3. **Mutasyon duzenegim kendi isimi sildi** — `git checkout --` izlenen dosyayi
   **HEAD'e** dondurur; W-4 degisikligim uctu, 4 mutasyon ayni bozuk durumu
   olctu. **Dosya-kopyasi** duzenegine gecildi.
4. **"Once tam oku" ihlali** — `EscalationRequest`'in ilk 56 satirini okuyup
   *"alan yok"* dedim; **alan zaten vardi**, mukerrer eklemistim.
5. **Olcum aletim yanlis olctu** — mutasyon siniflandiricim buyuk/kucuk harfe
   duyarliydi (`FAILED` != `*failed*`), 4 yakalanmis mutasyonu "kacti" okudu.
6. **Surum pini IKI yerde** (`CONTRACTS_VERSION.md` + `api/main.py`) — kapi
   yakaladi.
7. **Fikstur sorgu bicimiyle hizasiz** — sorguya 5. kolonu ekledim, fikstur
   4'lu demet uretmeye devam etti -> `IndexError`.
8. **1 yeni mypy hatasi** (walrus'lu kavrama, ayni ada iki atama).

### 10) MUTASYON: 3'u ILK TURDA KACTI ve TESTIMIN ZAYIF OLDUGUNU GOSTERDI

Toplam **48 mutasyon**, hepsi sonunda yakalandi. Ucu once kacti:
* **determinizm** — siralama ile esitlik-cozumu birbirinin **yedegiymis** (200
  karistirmayla olculdu); fazlalik kaldirildi, kalani kiran veri **arandi**.
* **asama damgasi** — testim damgayi degil **dis dedup**'i siniyormus.
* **indeks adresleri** — iki testim de `None` **kabul ediyordu**; alanin
...
* **C-2** `expert_feedback`'e `representative_tile_id` + **iki anlami AYIR**.
  Olculdu: kaskad bugun `bulk_approval_id` **varligindan** tetikleniyor
  (`feedback_handler:675`), ama o alan platformda *"5-50 ayri incelemeyi tek
  tikla onayladim"*, worker'da *"bu bir KARO GRUBUNUN temsilcisidir"* demek.
  Bugun zararsiz (grup yok); **suzgec-1 baglaninca sessizce yanlis yayilir.**
* **P-2** -> **W-5** -> (**W-1**, **W-2**, **W-6**)
* **RISK:** suzgec-1'i canliya baglamak **egitim setini zehirleyebilir**
  (`prototype_manager.py:76-88` kendi uyarisi) — F1/F4 kapilari **mutasyonla
  sinanmadan** baglanmayacak.

**TUR 4:** **P-4** platform enum aynasina `AUDIT_SAMPLE` + parite testi
(olculdu: worker **7** · platform **6** · kanonik **7**; platform testlerinde
...
* **Uzmanin gercekten 5 karo gordugu OLCULMEDI.** Kod canlida ve zincirin her
  ...
  yeni bir analiz kosumuyla gelir.
* **§14.17 bant genisligi** kalemi acik (tunel 0.47-0.90 MB/s, GDAL timeout 30 s
  — sabitler **SSRF sertlestirmesi**, gevsetilmeyecek).
* Devir notunun (1)'deki `44/35/35/22` sayilari ile FAISS is dagilimi hala
  **aciklanamadi** (§0.B (14)).

---

## 0.B — (2026-08-31, **yirmi dördüncü oturum: UZMANA GİDEN KANIT — ÖĞRENME DÖNGÜSÜ KİLİTLİ**)
