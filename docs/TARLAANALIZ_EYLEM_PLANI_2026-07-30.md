...
| ✅ **Akış ÖLÇÜLDÜ (2026-08-24) — zincir YARIM BAĞLI** | Üretim `rabbitmqctl`: `analysis_job_started` **2 tüketici** (önceki ölçümdeki **404 GİTTİ** → platform 7.8.0 tüketicisi ayakta) · `analysis_results` **2** · `expert_review_queue` **2**. 🔴 Ama `analysis_jobs` **0 tüketici** — platform buraya iş yayınlıyor (`worker_job_publisher.py:30`), **alan yok**. Yani platform tarafı sağlam, **worker bağlı değil** (kalem OP-1). Kod kusuru DEĞİL: worker o sunucuda koşamaz → SSH tüneli. Ayrıca bu dağıtımda RabbitMQ gerçekten yeniden yaratıldı ve **15 kuyruk korundu** → PR #428 üretimde DOĞRULANDI |
