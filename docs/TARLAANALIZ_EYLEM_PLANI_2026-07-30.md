...
| **DK-63** ✅ | ✅ **TAM KAPANDI 2026-08-30** (plat #484 · #485 · **#492**). ⚠️ 2026-08-29'da "kapandi" isaretlemek ERKEN BIR IDDIAYDI: yeniden analiz MUMKUN kilinmisti ama DURUST degildi — gorevi `EXPERT_REJECTED`e tasimanin TEK yolu IKI UZMANIN GECERLI incelemesini REDDETTIRMEKTI ve o ret KALICI musteri kaydina yazildi; kalemin kendi metni bu zarari zaten yaziyordu. **#492** `ExpertReviewStatus.SUPERSEDED` ekledi: inceleme kapanir ama YARGI TASIMAZ. `PENDING_REVIEW` artik YALNIZ yeniden analiz icin acik; acik inceleme varken YENI bir veri seti sevk edilemez (409). Uc mutasyonla dogrulandi — biri ILK YAZDIGIM TESTI KIRMADI (`str(stmt)` bagli parametreyi gostermez, yani hicbir sey olcmuyordum; `stmt.compile().params` ile duzeltildi). Ozgun kapanis notu: (plat #484 + #485). Cozum secenek ①/② degil UCUNCUSU oldu: `Dataset.is_ready_for_reanalysis` — kosullar `is_ready_for_analysis` ile AYNI, yalniz beklenen durum `ANALYZED`; gozetim zinciri GERI ALINMAZ (`record_reanalysis_dispatch()` durumu `ANALYZED` birakir, turlar manifest'te LISTE olarak birikir). Yeniden analiz istemciden GELMEZ, sunucuda gorev durumundan TURETILIR (yalniz `EXPERT_REJECTED`, degilse 409). Uc artik opsiyonel `dataset_id` alir → ESKI ucus acikca hedeflenebilir (`sim-faz4d-sevk.py … --dataset`). ⚠️ Arkasindan CIKAN kusur: uc gecisi sevkten ONCE commit ediyordu; `FAILED` donunce gorev `ANALYZING`de KILITLENIYORDU (#485). Ozgun kayit: **`PENDING_REVIEW`daki bir goreve DAHA IYI veri gelirse, yeniden analizin tek yolu eski analizi REDDETTIRMEK.** Olculdu 2026-08-29, **gercek musteri siparisinde** (`24cceb52`, Karaburun 102/1): Ucus-2 yuklendi, sevk **409** verdi — *“Gorev durumu sevke uygun degil: PENDING_REVIEW”*. `mission.py:88` → `PENDING_REVIEW` yalnizca `DONE / EXPERT_REJECTED / FAILED / CANCELLED`’a gidebilir; bunlardan sevk edilebilir olan `EXPERT_REJECTED` ve `FAILED`. ℹ️ **Yeniden ucus (KR-028 REFLY) DESTEKLENIYOR** — sevk ucu en guncel veri setini secer, `EXPERT_REJECTED → ANALYZING` gecisini kendi yapar ve AV2 muafiyetini de kendi verir (`admin_dashboard.py` sevk ucu). Bosluk yalnizca **inceleme SIRASINDA** yeni veri gelmesi hâlinde. 💥 Zarar: uzman, analiz kalitesiyle ilgisi olmayan bir sebeple **gecerli bir analizi reddetmek** zorunda kaliyor ve bu kalici musteri kaydina yaziliyor. Bugun ret ZATEN dogruydu (Ucus-1 kapsamasi %22.4 + maske oncesi uretilmis), yani zarar gizlendi — ama dogru bir ilk analiz varken ikinci ucus gelirse ret **yanlis** olacak. ✅ Secenekler: ① `PENDING_REVIEW`’a yeni veri seti gelince incelemeyi **SUPERSEDED** isaretleyip gorevi `ANALYZING`’e almak (yeni durum gerektirmez, mevcut gecise yeni bir tetik) ② sevk kumesine `PENDING_REVIEW` eklemek (⚠️ acik incelemeyi ortada birakir). ① tercih edilir. | platform mission + admin sevk |
| **DK-64** 🟠 | 🔶 **KADEME 1 KAPANDI 2026-08-29** (plat **#487**): ciftci artik **bitki ortusu / acik toprak** dokumunu goruyor ve arayuz "agac" DEMEZ — otun dahil oldugunu ACIKCA yazar. Terim korumasi TESTE bagli (bilesen "agac" derse CI kirilir). 🔴 **KADEME 2 ACIK:** agac/ot ayrimi CHM ister; uretici HALA KURULU DEGIL, bugune dek elle yapildi → sahada uretilemez. Ozgun kayit: **`canopy_cover_ratio` “agac orani” DEGIL “yesil orani” — yabanci ot kanopi sayiliyor ve olcumu SAPTIRIYOR.** Bitki-ortusu maskesi `NDVI >= 0.40` ile calisir; ot da yesildir ve esigi gecer. NDVI yesil bir otu yesil bir yapraktan **ayiramaz** — esik ayari degil, bandin tasidigi bilginin siniri. 📏 Olculdu 2026-08-29, nokta bulutundan uretilen CHM ile (`odm_filterpoints/point_cloud.ply`, kendi pozitif/negatif kontrolunu gecti: tepe 3.77 m ≈ fistik agaci, zemin medyani 0.087 m ≈ 0): **Ucus-1** agac %22.3 · ot %5.5 · toprak %72.2 — **Ucus-2** agac %6.5 · ot **%13.6** · toprak %79.9. Yani Ucus-2’de “kanopi” denen %20.1’in **ucte ikisinden fazlasi ot**. Ortalama NDVI’ye etkisi iki ucusta da ayni yonde ve neredeyse ayni buyuklukte: yesil 0.6365 / 0.6337 → **yalniz agac 0.6682 / 0.6663** (fark **+0.0317 / +0.0326**). Ot olcumu sistematik olarak ~0.03 asagi cekiyor. ℹ️ Bu bahcede sinif degistirmedi (ikisi de “iyi” bandinda) ama sinif sinirina yakin bir bahcede cevirir. ✅ Cozum yapisal maske: `CHM = DSM − DTM` → agac (NDVI≥esik **ve** h>~1 m) / ot (NDVI≥esik **ve** h≈0) / toprak. Yukseklik esigi VERIDEN secildi (Otsu, yalniz yesil pikseller uzerinde): iki ucusta bagimsiz olarak **1.52 m / 1.46 m**; agac taci medyani **2.74 m / 2.71 m** — farkli yukseklik, farkli nokta bulutu, ayni agaclar. 🎁 Yan kazanc: maskeli olcum uctan uca **tekrarlanabilir** oluyor — yesil bazli iki ucus 0.0028 sapiyor, **agac bazli 0.0019**. Ve urun sahibinin istedigi *“bahcenizin %…’i agac, %…’i toprak, %…’i ot”* dokumu ancak boyle **olculmus** olur. ⚠️ Ciftciye sunmadan once tarla poligonuyla KIRPILMALI: Ucus-2 ortofotosu 32.8 donumu kapliyor, kayitli tarla **29.0 donum** — kirpilmazsa bahce disindaki topragi ciftcinin bahcesi sayariz. ➕ **Ayni kararin ikinci yuzu — cozunurluk:** `pistachio.yaml` `gsd_cm: [0.5, 1.5]` istiyor; ortofotoyu **5 cm/px** uretiyoruz, yani sart ortofoto adiminda ATILIYOR (Ucus-1’in 1.38 cm ham GSD’si kullanilmiyor). Bugun zarari GIZIL — `INDICES_ONLY` kipindeyiz ve DIRECT kartlar kanopi olcekli; ama zararli tespiti dal/yaprak olceginde calisacaksa bu tavan. 1.5 cm’e cikmak dosyayi ~11 kat buyutur (52 MB → ~580 MB) ve karo sayisini degistirir — CHM karariyla BIRLIKTE verilmeli. | worker (maske + CHM) + platform (sunum) |
| **DK-65** ✅ | ✅ **KAPANDI 2026-08-29** — kirpma uctan uca KOSTU. `analysis_job.v1` 7.9.0 `field_boundary_geojson` (opsiyonel, 0 breaking) · platform `fields.boundary_geojson` (JSONB) okur · worker `field_clip.py` bant duzeyinde kirpar. Uretim olcumu: **Ucus-1 %4.5**, **Ucus-2 %57.9** tarla disi — elle olculen %56 ile **1.9 puan** icinde uyustu (iki bagimsiz yontem). Iki ucus da maskeli+kirpilmis: **0.666** (ortu %27.2) ve **0.646** (ortu %19.1). ⚠️ Kirpma sagligi neredeyse degistirmedi (0.650→0.646) ama ORTU oranini %20.6→%19.1 dusurdu — bahce DISI, bahcenin kendisinden daha yesildi; nedeni OLCULMEDI. Ozgun kayit: **Ucus goruntusunun %56'si TARLA SINIRLARININ DISINDAYDI — kirpma olmadan verilen her yuzde ciftcinin bahcesini yanlis anlatir.** Olculdu 2026-08-29 (gercek siparis `24cceb52`, tarla poligonu 28.93 donum vs kayitli 29.00 — %0.24 sapma, pozitif kontrol): **Ucus-2** ortofotosu 32.80 donum kapliyor, bunun **18.35 donumu (%56.0)** tarla disi; tarla icinde olculen 14.45 donum (tarlanin %49.8'i). **Ucus-1**: 6.22 donumun 0.30'u (%4.9) disarida. 💥 Etkisi buyuk: kirpma ONCESI Ucus-2 dokumu agac %6.5 · ot %13.6 · toprak %79.9 idi; kirpma SONRASI **agac %13.4 · ot %5.3 · toprak %81.3** — yani “ot” sanilanin cogu bahcenin DISINDAKI alandi. Kirpilmamis sayi sunulsaydi ciftciye bahcesi ciddi bicimde yanlis anlatilirdi. ⚠️ Worker sinir bilgisini **HIC ALMIYOR**: `analysis_job.v1` semasinda geometri yok ve worker kodunda hicbir `clip`/`mask` cagrisi yok (olculdu) — yani bugun kirpma **hicbir yerde** yapilmiyor. ⚠️ Ikinci tuzak: `fields.boundary` (PostGIS) **NULL**; poligon `fields.boundary_geojson` (JSONB) icinde — uzamsal sorgu yazan sessizce bos sonuc alir. ✅ Yon: sinir `analysis_job`a eklenir ve worker olcumden ONCE kirpar (ya da platform ingest'te kirpar). ➕ Seffaflik (urun sahibi karari): sinir disina tasan oran ciftciden GIZLENMEZ, ek bilgi olarak sunulur — pilotun sinir disina ciktigini bilmesi dogru. | worker + platform + contract (analysis_job) |
