...
| **P18** | platform | **SSOT metni senkronu (D16-b2 turu)** — `docs/TARLAANALIZ_SSOT_v1_2_0.txt` contract'ta 1906→1954 satır oldu (KR-019 eskalasyon katmanı, KR-092 fail-closed düzeltmesi + IDOR/yükleyici maddeleri, KR-072 `evidence_bundle_ref`). Hedefteki tek "yalnız orada" satır **düzeltilen eski `clamp` satırı** — kayıp yok, üzerine yazılmalı. I-3 bayt-özdeşliği bu PR'la geri kurulur | D16-b2 · `sync_kr_corpus.py --check` |
| **W9** | worker | **Aynı SSOT metni senkronu** (`docs/TARLAANALIZ_SSOT_v1_2_0.txt`). Ek olarak: worker'ın kendi `docs/reference/kr_registry.md`'si `POINTER_OK` — dokunulmaz | D16-b2 |
